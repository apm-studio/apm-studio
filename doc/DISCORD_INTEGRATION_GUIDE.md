...
- it does not create, edit, save, or publish Agent Roster assets
...
- config storage: `~/.agent-roster/discord-config.json`
- channel/role mappings: `~/.agent-roster/discord-mappings.json`
...
- config and mapping files are written under `~/.agent-roster` with private file permissions
...
- `/act message` participant autocomplete for Team thread input, scoped to the mapped Team channel
...
- `/performer new` creates a fresh standalone Agent thread and is accepted only in mapped Agent thread channels.
- `/act participants` lists participants for the current Team and is accepted only in mapped Team thread channels.
- `/act message participant:<participant> message:<text>` sends a message to one participant in the current Team thread. The participant option uses channel-scoped autocomplete from the mapped Team only.
- `/act sync` backfills recent visible participant messages and is accepted only in mapped Team thread channels.
...
- Team thread history sync is assistant-output-only per participant; Discord user input is posted by the bot as normalized `[Studio User -> Participant]` text, while runtime-injected user prompts such as teammate wake/direct-message prompts stay hidden
...
- Team thread sync keeps one watcher per Discord Team thread, starts that watcher immediately after `/act message`, also wakes it from Team runtime thread-update events, extends the watcher when the thread is touched again or new participant output is synced, refreshes the Team thread session list on every poll, emits typing while any participant session or Team participant runtime status is still busy or retrying, and waits for a stable-idle window before stopping
- Team thread input uses `/act message`; direct text messages in Team channels are not routed
- Discord Team channels sync visible text from all participant sessions in that Team thread
- Discord rejects additional Team chat if any participant session in that Team thread is running, retrying, or waiting on permission/question input
...
- Team thread input must come through `/act message`
