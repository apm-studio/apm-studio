...
- browser-only UI preferences may use localStorage when they do not represent workspace/package content. Last selected top-level mode is stored through `src/store/workspace/workspace-mode-storage.ts` so refresh can return to Import, Export, or Studio Agent without writing `.apm-studio/workspace.json`.
