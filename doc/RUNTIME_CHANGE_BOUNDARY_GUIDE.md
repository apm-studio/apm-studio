...
- Export may list unmanaged target definition files, but it must only pair a Studio package with a target definition when `.apm-studio/projections/apm-sync.json` records the managed `packageId`; do not guess package-to-target matches from names, slugs, or file paths.
