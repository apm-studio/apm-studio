...
- Agent instructions, Skill, model, variant, MCP, binding, and delivery mode changes. Model changes affect Studio Agent runtime projection only; external target sync must omit model selection.
...
- Agent projection uses Agent instructions plus referenced Skills, MCP bindings, and Studio-only model settings
...
- external assistant files are managed by the Export target-sync page, not by normal Studio save, startup, chat projection, or Team projection
- `GET /api/apm/targets` must be a dry-run target/tooling status calculation and must not write assistant files. Target summaries include supported sync units, sync strategy, output hints, APM CLI target detection status from `apm targets --json --all` when a CLI runner is available, Studio-managed current items, and read-only target definition files so the Export UI can compare APM Studio with the selected target.
- `GET /api/apm/audit` must be a read-only APM integrity check. It delegates to the selected Microsoft APM CLI with `apm audit --ci --no-policy -f json`, preserves JSON output even when the CLI exits non-zero, and surfaces lockfile consistency, deployed-file drift, and content-integrity failures without attempting Studio-side remediation.
- `POST /api/apm/sync` is the manual path that syncs the selected APM primitive unit (`agents`, `instructions`, `skills`, `prompts`, `commands`, `hooks`, or `mcp`) to one or more targets. APM primitive units follow the target capability registry for Codex, Claude, OpenCode, Cursor, Windsurf, Copilot, Gemini, and Agent Skills. Prompt and command sync both assemble `.apm/prompts/*.prompt.md`; the target capability decides whether the APM CLI writes prompts or commands. MCP sync assembles `apm.yml` `dependencies.mcp` entries, including self-defined command/args/env/url details imported from MCP config files. The route rejects unknown sync units, receives only Studio packages staged as export changes, and prefers the external Microsoft APM CLI through a configured command, local `apm`, or `uvx --from git+https://github.com/microsoft/apm.git apm`. Studio-native TypeScript projection is a fallback only when no APM CLI runner is available and only for supported agent and skill units; once a CLI runner is selected, CLI failure must be reported instead of masked by Studio fallback output.
- target sync writes do not require OpenCode `dispose`
- The Studio UI exposes this manual external-assistant sync as the top-level `Export` mode; that mode remains a manual target-sync boundary and does not make external assistant files part of normal workspace save or runtime preparation. The left sidebar owns the active APM scope through a `User` section above `Workspaces`, shared with Import. The screen should keep the flow simple: scoped Studio source cards, then one selected target panel where drag/drop or `Add to target` only stages export changes until the header `Save` action commits file writes. User-scope packages are copied into Workspace before target export. The header `Revert` action clears staged package-copy and target-export changes without writing target files. Export staging, save results, and target refresh responses are workspace-local; workspace changes must clear staged UI state and ignore stale in-flight target responses from the previous working directory.
- Export may list unmanaged target definition files, but it must only pair a Studio package with a target definition when `.apm-studio/projections/apm-sync.json` records the managed `packageId`; do not guess package-to-target matches from names, slugs, or file paths. Managed current packages must remain visible in the target panel after refresh even when no package is staged; multiple managed target definition files for the same package must all be treated as package-owned rather than target-only. Importing a target-only definition into Workspace is a package import operation, not a target-sync write, and must leave the original target file unmanaged until the user stages and saves an Export sync.
- The target-sync screen currently lives under `src/features/target-export`; it should not be reintroduced through workspace/canvas feature barrels because external assistant target sync is not a workspace editing surface.
...
- production client serving must expose the full Vite `client/` output before the SPA fallback, including root files copied from `public/` such as favicons, `site.webmanifest`, installer scripts, and APM Studio logo assets
