...
- Agent primitive links in workspace snapshots use `instructionRef` and `skillRefs`; no older primitive-link aliases are part of the supported workspace schema
...
- this is the canonical package boundary for agents, instructions, skills, MCP requirements, Studio Agent model selection, and external target sync
...
- Studio-managed packages must keep managed APM source primitives under the package's own `.apm/agents`, `.apm/instructions`, and `.apm/skills` so the package root remains inspectable and compatible with APM-shaped source tooling. Prompts, commands, and hooks can exist in imported upstream packages; Studio preserves/counts them and can sync them through the CLI-first target path, but does not expose them as editable Manage primitives. MCP syncability is counted from `apm.yml` `dependencies.mcp` entries, not from generated target MCP files.
...
- Studio Agent name, description, Agent Body, Instruction, Skills, model, and MCP requirements are package-affecting fields. The Agent description must flow into the generated `.apm/agents/<name>.agent.md` frontmatter `description`; Agent Body flows into the `.agent.md` body; reusable Instructions remain separate `.apm/instructions/*.instructions.md` primitives; Skills remain separate `.apm/skills/*/SKILL.md` primitives rather than custom agent frontmatter. Model selection is stored for Studio Agent runtime only and must not be emitted into external assistant projection artifacts.
- Dragging an installed Instruction, Skill, or MCP package onto an Agent attaches the package as an `apm-package/<scope>/<packageId>` ref. When the Agent package is saved, package refs must resolve back through the APM package service and materialize the referenced `.apm/instructions` or `.apm/skills` source into the Agent package; do not treat these local package refs like unresolved external registry refs.
- target management rule: do not bundle the Python `microsoft/apm` CLI into Studio's npm package or require it at install time. Studio treats APM as the package/spec shape and performs external target sync through a CLI-first pipeline: use `APM_STUDIO_APM_CLI` when configured, then `apm`, then `uvx --from git+https://github.com/microsoft/apm.git apm`, and fall back to Studio-native TypeScript projection only for supported Studio Agent, agent, and skill sync units. Instructions, prompts, commands, hooks, and MCP are CLI-only; if the CLI is unavailable or produces no project-scoped artifact, Studio reports a skipped result instead of inventing a fallback format.
- shared APM package/import contracts live in `shared/apm-contracts.ts`; target-sync contracts, sync-unit helpers, and target result shapes live in `shared/apm-sync-contracts.ts`. Manage UI and server sync services should import sync shapes from the sync contract file instead of widening the package contract boundary.
- external target sync reads from this boundary
- Import writes source-reference listings into this boundary as APM packages in the selected scope: the current workspace by default, or the APM user scope (`~/.apm`) when the user chooses User
- package APIs use the same `workspace` / `user` scope contract for list, read, write, import, validation, and sync; route handlers must resolve scope through `requestApmPackageWorkingDir` rather than reaching directly into workspace path helpers
...
- Import GitHub source catalogs should use a repo-specific conversion layer to expose source contents as package candidates before import; tree discovery uses the GitHub tree/raw APIs and may fall back to GitHub codeload tarballs when the GitHub API is rate-limited, but it must not shallow clone repositories in the request path. Only explicit imports write local APM packages. Catalog-only discovery lives in `server/services/apm-package/github-source-catalog.ts`; candidate scanning orchestration lives in `server/services/apm-package/github-import-candidates.ts`; candidate ids/source metadata live in `github-import-candidate-ids.ts`; candidate manifest assembly lives in `github-import-candidate-builders.ts`; package preview/import writes live in `server/services/apm-package/github-import.ts`.
...
- GitHub source imports preview detected APM packages, managed `.apm/` primitives, `SKILL.md` folders, Claude agent Markdown, Codex TOML, instruction Markdown, and MCP configs before writing local packages; prompt Markdown, hooks, and commands are not standalone Studio-managed candidates. Full APM package imports must preserve `.apm/prompts/*.prompt.md` and `.apm/hooks/*.json`; APM command sync uses those same prompt files for command-capable targets. Standalone MCP config imports must preserve self-defined server details such as command, args, env, URL, headers, and transport in `dependencies.mcp` so CLI-first target sync can export the same server configuration. The original repo/ref/path should stay in the manifest source metadata and `x-apm.agent.derivedFrom` when applicable.
...
- inline agent bodies imported from Codex TOML or other assistant-native formats stay in APM native `agents` fields and are mirrored into `x-apm.agent.agentBody` for projection. Reusable Instruction refs are stored separately as `x-apm.agent.instructionRef`.
...
- new UI, import, package, injection, and authoring work must go through package reads/writes
...
- `server/routes/apm/packages.ts` owns APM package listing, reading, writing, primitive read/write under the package `.apm/` tree, and manifest validation routes.
...
- CLI-produced and Studio-fallback target artifacts must share the same ownership manifest at `.apm-studio/projections/apm-sync.json` so Manage target inspection, overwrite protection, and current-item summaries use one source of truth.
