...
- this is the canonical package boundary for agents, instructions, skills, MCP requirements, Studio Run model selection, and external target sync
...
- Studio-managed packages must keep managed APM source primitives under the package's own `.apm/agents`, `.apm/instructions`, and `.apm/skills` so the package root remains inspectable and compatible with APM-shaped source tooling. Prompts, hooks, and commands can exist in imported upstream packages, but Studio does not expose them as editable primitives.
...
- Manage-mode Agent name, description, Agent Body, Instruction, Skills, model, and MCP requirements are package-affecting fields. The Agent description must flow into the generated `.apm/agents/<name>.agent.md` frontmatter `description`; Agent Body flows into the `.agent.md` body; reusable Instructions remain separate `.apm/instructions/*.instructions.md` primitives; Skills remain separate `.apm/skills/*/SKILL.md` primitives rather than custom agent frontmatter. Model selection is stored for Studio Run only and must not be emitted into external assistant projection artifacts.
- injection rule: do not bundle the Python `microsoft/apm` CLI into Studio's npm package or require it at install time. Studio treats APM as the package/spec shape and performs external target sync through a CLI-first pipeline: use `APM_STUDIO_APM_CLI` when configured, then `apm`, then `uvx --from git+https://github.com/microsoft/apm.git apm`, and fall back to Studio-native TypeScript projection only for supported agent and skill sync units.
- shared APM package/import contracts live in `shared/apm-contracts.ts`; Inject/target-sync contracts, sync-unit helpers, and target result shapes live in `shared/apm-sync-contracts.ts`. Inject UI and server sync services should import sync shapes from the sync contract file instead of widening the package contract boundary.
...
- Import GitHub source catalogs should use a repo-specific conversion layer to expose source contents as package candidates before import; tree discovery may fall back to GitHub codeload tarballs when the GitHub API is rate-limited, and only explicit imports write local APM packages. Catalog-only discovery lives in `server/services/apm-package/github-source-catalog.ts`; candidate scanning orchestration lives in `server/services/apm-package/github-import-candidates.ts`; candidate ids/source metadata live in `github-import-candidate-ids.ts`; candidate manifest assembly lives in `github-import-candidate-builders.ts`; package preview/import writes live in `server/services/apm-package/github-import.ts`.
- GitHub source imports preview detected APM packages, managed `.apm/` primitives, `SKILL.md` folders, Claude agent Markdown, Codex TOML, instruction Markdown, and MCP configs before writing local packages; prompt Markdown, hooks, and commands are not Studio-managed candidates. The original repo/ref/path should stay in the manifest source metadata and `x-apm.agent.derivedFrom` when applicable
...
- CLI-produced and Studio-fallback target artifacts must share the same ownership manifest at `.apm-studio/projections/apm-sync.json` so Inject target inspection, overwrite protection, and current-item summaries use one source of truth.
