...
import { assetFilePath, danceAssetDir, ensureApmAssetDir, getGlobalCwd, getRegistryPackage, readAsset } from './apm-asset-source.js'
import {
    existsInRegistry,
    loadLocalAssetByUrn,
    parseApmAsset,
    parseApmAssetUrn,
} from './apm-asset-source.js'
...
// Re-export auth helpers so apm-asset-service.ts can import from one place
export { readAuthUser as readApmAuthUser, clearAuthUser as clearApmAuthUser } from './apm-asset-source.js'
...
function finalizeAsset(asset: Record<string, unknown>) {
    return parseApmAsset(asset) as Record<string, unknown>
}
...
async function ensurePerformerDanceDependencies(cwd: string, payload: Record<string, unknown>) {
    const parsed = parseApmAsset(payload)
...
export async function saveLocalStudioAsset(options: {
    ...
    await ensureApmAssetDir(options.cwd)
