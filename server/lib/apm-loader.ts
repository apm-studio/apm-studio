...
export function resolveApmCommand(): string[] {
    const localBuiltCli = path.resolve(__dirname, '..', '..', 'cli.js')
    return [
        resolvePackageBin('apm-studio', 'apm-studio')
            || localBuiltCli
            || 'apm-studio',
    ]
}
...
export async function ensureApmLoaderServer(cwd: string): Promise<{
    available: boolean
    serverName: string
    toolName: string
}> {
    const serverName = apmLoaderServerName(cwd)
    ...
    if (!existing) {
        const config = {
            type: 'local',
            command: resolveApmCommand(),
            enabled: true,
            environment: {
                APM_STUDIO_PROJECT_DIR: path.resolve(cwd),
            },
