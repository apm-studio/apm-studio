...
const ensureApmStudioDirMock = vi.fn()
...
vi.mock('../../lib/apm-studio-paths.js', () => ({
    ensureApmStudioDir: ensureApmStudioDirMock,
}))
...
describe('initializeStudioProject', () => {
    ...
    it('primes the requested workspace directory for CLI startup restore', async () => {
        const { initializeStudioProject } = await import('./service.js')

        await expect(initializeStudioProject('/tmp/workspace/')).resolves.toBe('/tmp/workspace')

        expect(ensureApmStudioDirMock).toHaveBeenCalledWith('/tmp/workspace')
