...
export type TeamSessionSettlementOutcome =
    | { kind: 'idle' }
    | { kind: 'timeout'; message: string }
    | { kind: 'fatal_error'; message: string }

export function formatTeamSessionError(error: unknown) {
    return error instanceof Error ? error.message : String(error)
}

export async function resolveTeamSessionSettlementOutcome(
...
): Promise<TeamSessionSettlementOutcome> {
