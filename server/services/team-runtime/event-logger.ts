/**
 * event-logger.ts — Append-only event log for Team Thread
 *
 * PRD §6.3: Events are stored as append-only .jsonl files.
 * Path: ~/.apm-studio/workspaces/<workspaceId>/team-runtime/<teamId>/<threadId>/events.jsonl
 */

import { promises as fs } from 'node:fs'
import { join, dirname } from 'node:path'
import type { MailboxEvent } from '../../../shared/team-types.js'
...
export class EventLogger {
    ...
    constructor(workspaceId: string, teamId: string, threadId: string) {
        this._workspaceId = workspaceId
        this._teamId = teamId
        this._threadId = threadId
        this.logPath = join(
            workspaceTeamRuntimeDir(workspaceId, teamId, threadId),
            'events.jsonl',
        )
    }
