...
import { parseTeamSessionOwnerId } from '../../lib/session-execution.js'
import { buildTeamContext } from './team-context-builder.js'
import { getStaticTeamTools } from './team-tools.js'
...
describe('collaboration context rewrite', () => {
    it('builds stable collaboration context without model-facing IDs', () => {
        const context = buildTeamContext(teamDefinition, 'Lead')
    ...
    it('lists only outbound or bidirectional teammates as valid message recipients', () => {
        const oneWayTeamDefinition: TeamDefinition = {
            id: 'team-one-way',
            name: 'One Way Team',
            participants: {
                Lead: {
        ...
        const researcherContext = buildTeamContext(oneWayTeamDefinition, 'Researcher')
        ...
        const leadContext = buildTeamContext(oneWayTeamDefinition, 'Lead')
    ...
    it('parses team participant owner IDs from session context', () => {
        expect(parseTeamSessionOwnerId('team:team-review:thread:thread-1:participant:Lead')).toEqual({
            teamId: 'team-review',
            threadId: 'thread-1',
            participantKey: 'Lead',
        })
