...
describe('resolveTeamSessionSettlementOutcome', () => {
    beforeEach(() => {
        vi.clearAllMocks()
    })
    ...
    it('returns timeout when the session never settles', async () => {
        ...
        const { resolveTeamSessionSettlementOutcome } = await import('./team-session-settlement.js')
        await expect(resolveTeamSessionSettlementOutcome(
