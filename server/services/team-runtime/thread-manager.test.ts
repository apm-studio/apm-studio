...
import type { TeamDefinition, BoardEntry } from '../../../shared/team-types.js'

async function makeTempStudioDir() {
    return fs.mkdtemp(path.join(os.tmpdir(), 'apm-studio-team-runtime-'))
}
...
const baseTeamDefinition: TeamDefinition = {
    id: 'team-review',
    name: 'Review Team',
    participants: {
        Lead: {
...
describe('ThreadManager full-rewrite persistence', () => {
    it('hard-resets incompatible persisted thread snapshots', async () => {
        ...
        try {
            ...
            const threadDir = path.join(studioDir, 'workspaces', workspaceId, 'team-runtime', teamId, threadId)
            ...
            expect(manager.listThreadIds(teamId)).toEqual([])
    ...
    it('restores board entries from board.json while keeping pending mailbox state from thread snapshots', async () => {
        ...
        try {
            ...
            await fs.writeFile(path.join(threadDir, 'thread.json'), JSON.stringify({
                ...
                teamDefinition: baseTeamDefinition,
            }, null, 2))
    ...
    it('retires current participant sessions when agent refs change or participants are removed', async () => {
        const studioDir = await makeTempStudioDir()
        try {
            const workspaceId = 'workspace-3'
            const { ThreadManager } = await importThreadManagerWithStudioDir(studioDir)
            const manager = new ThreadManager(workspaceId, '/tmp/workspace')
            const thread = await manager.createThread(baseTeamDefinition.id, baseTeamDefinition)
            ...
            await manager.syncThreadTeamDefinition(thread.id, {
                ...baseTeamDefinition,
                participants: {
                    Lead: {
                        agentRef: { kind: 'draft', draftId: 'lead-v2' },
                    },
                },
                relations: [],
            })

            expect(manager.getAgentSession(thread.id, 'Lead')).toBeNull()
            expect(manager.getAgentSession(thread.id, 'Researcher')).toBeNull()
