/**
 * safety-guard.ts — Safety & Guard for Team runtime
 *
 * PRD §16: Event budget, loop detection, timeout, idle detection, permission checks.
 */

import type { TeamRelation, TeamSafetyConfig, TeamThread, BoardEntry, MailboxEvent } from '../../../shared/team-types.js'
...
export interface SafetyConfig {
    /** Maximum total events per Team thread */
    maxEventsPerThread: number
    /** Maximum messages between any pair of agents */
...
export class SafetyGuard {
    ...
    /**
     * Create a SafetyGuard from a TeamSafetyConfig (from TeamDefinition.safety).
     * Maps field names from the shared type to the internal SafetyConfig.
     */
    static fromTeamSafety(teamSafety?: TeamSafetyConfig): SafetyGuard {
        if (!teamSafety) return new SafetyGuard()
        return new SafetyGuard({
            maxEventsPerThread: teamSafety.maxEvents,
            maxMessagesPerPair: teamSafety.maxMessagesPerPair,
            maxBoardUpdatesPerKey: teamSafety.maxBoardUpdatesPerKey,
            quietWindowMs: teamSafety.quietWindowMs,
            loopDetectionThreshold: teamSafety.loopDetectionThreshold,
            ...(typeof teamSafety.threadTimeoutMs === 'number'
                ? { threadTimeoutMs: teamSafety.threadTimeoutMs }
                : {}),
        })
    }

    /**
     * Check total event budget for the Team thread.
     */
    checkEventBudget(event: MailboxEvent): { ok: boolean; reason?: string } {
        ...
        if (current > this.config.maxEventsPerThread) {
            return { ok: false, reason: `Event budget exceeded (${this.config.maxEventsPerThread} max). Thread should be completed.` }
        }
        return { ok: true }
    }

    /**
     * Check messages between a pair of agents.
     */
     ...
     */
    checkTimeout(thread: TeamThread): { ok: boolean; reason?: string } {
     ...
     */
    checkPermission(from: string, to: string, relations: TeamRelation[]): { ok: boolean; reason?: string } {
