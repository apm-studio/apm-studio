 ...
 *
 * PRD §15.3: Team Runtime Engine tells a participant WHAT HAPPENED, not WHAT TO DO.
 * Wake prompts are informational summaries injected into participant sessions.
 */

import type { TeamDefinition, MailboxMessage } from '../../../shared/team-types.js'
...
export function resolveWakePrompt(
    ...
    const participantName = (key: string | null | undefined) => {
        ...
        return teamDefinition?.participants[key]?.displayName || key
    }
...
export function buildWakePrompt(
    ...
    return renderWakePrompt(resolveWakePrompt(target, mailbox, teamDefinition))
}
