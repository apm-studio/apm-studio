/**
 * team-tools.ts — Team runtime custom tool definitions
 *
 ...
 * Team/thread/participant identity is resolved from the current session.
 */
...
/**
 * The generic Team runtime tools.
 * These are static and session-bound.
 * The workingDir is baked in at write time (changes per workspace, not per thread).
 */
export function getStaticTeamTools(workingDir: string): Array<{ name: string; content: string }> {
...
export default tool({
    ...
    async execute(args, context) {
        ...
        const res = await fetch(\`${base}/api/team/session/\${encodeURIComponent(sessionID)}/message-teammate?workingDir=${wd}\`, {
            method: "POST",
...
export default tool({
    ...
    async execute(args, context) {
        const sessionID = context.sessionID
        const res = await fetch(\`${base}/api/team/session/\${encodeURIComponent(sessionID)}/update-shared-board?workingDir=${wd}\`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
...
export default tool({
    ...
    async execute(args, context) {
        ...
        const suffix = qs ? "&" + qs : ""
        const res = await fetch(\`${base}/api/team/session/\${encodeURIComponent(sessionID)}/list-shared-board?workingDir=${wd}\` + suffix)
        const data = await res.json()
        if (!data.ok) return data.error
...
export default tool({
    ...
    async execute(args, context) {
        ...
        const res = await fetch(\`${base}/api/team/session/\${encodeURIComponent(sessionID)}/get-shared-board-entry?workingDir=${wd}&\` + params.toString())
        const data = await res.json()
        if (!data.ok) return data.error
...
export default tool({
    ...
    async execute(args, context) {
        ...
        const sessionID = context.sessionID
        const res = await fetch(\`${base}/api/team/session/\${encodeURIComponent(sessionID)}/wait-until?workingDir=${wd}\`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
