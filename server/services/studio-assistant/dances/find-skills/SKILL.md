---
name: find-skills
description: Finds, compares, and applies existing external skills or Skill Packs before creating a new one. Use when the user asks to find a skill, search skills.sh, recommend an existing skill, or install/apply a GitHub or skills.sh Skill Pack.
compatibility: Designed for the Agent Roster built-in assistant projection.
---

# Find Skills

Use this skill when the user likely wants an existing external skill, not a brand-new local Skill Pack from scratch.

## Intent Split
- If the user wants a new Skill Pack draft, an updated `SKILL.md`, or a custom local bundle, use `studio-assistant-skill-creator-guide` instead.
...
- Then consider Agent Roster registry matches.
- Then consider `skills.sh` or GitHub Skill Pack candidates.
...
## Apply In Studio
- Use `addDanceFromGitHub` for GitHub or `skills.sh` Skill Pack installs with `owner/repo` or `owner/repo@skill`.
- If the target Agent is already known and the installed Skill Pack URN is known from discovery hints, you may install first and then attach it in the same tool call with `addDanceUrns`.
