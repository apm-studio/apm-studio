...
## Apply In Studio
- Use `addDanceFromGitHub` for GitHub or `skills.sh` Skill installs with `owner/repo` or `owner/repo@skill`.
- If the target Agent is already known and the installed Skill URN is known from discovery hints, you may install first and then attach it in the same tool call with `addDanceUrns`.
