...
describe('ensurePerformerProjection source boundaries', () => {
    ...
    beforeEach(async () => {
        ...
        compilePerformerMock.mockReset().mockResolvedValue({
            ...
            agentNames: { build: '8pm-studio/workspace/hash/performer-1--build' },
            agentPaths: {
                build: path.join(workingDir, '.opencode', 'agents', '8pm-studio', 'workspace', 'hash', 'performer-1--build.md'),
            },
            agentContents: {
            ...
            skills: [],
            projectionHash: 'hash',
            allFiles: ['.opencode/agents/8pm-studio/workspace/hash/performer-1--build.md'],
        })
    ...
    it('does not write Codex project agent definitions during normal Studio projection', async () => {
        const buildContent = '---\ndescription: "Agent: Performer"\nmode: primary\n---\n\nbody'
        const buildPath = path.join(workingDir, '.opencode', 'agents', '8pm-studio', 'workspace', 'hash', 'performer-1--build.md')
        const codexRelativePath = '.codex/agents/8pm_studio_performer_1_deadbeef.toml'
        const codexPath = path.join(workingDir, codexRelativePath)
        const codexContent = [
            'name = "8pm_studio_performer_1_deadbeef"',
        ...
        compilePerformerMock.mockResolvedValueOnce({
            ...
            codexAgentName: '8pm_studio_performer_1_deadbeef',
            ...
            projectionHash: 'hash',
            allFiles: [
                '.opencode/agents/8pm-studio/workspace/hash/performer-1--build.md',
                codexRelativePath,
            ],
        })
    ...
    it('syncs Codex project agents immediately without rewriting OpenCode agent files', async () => {
        const buildPath = path.join(workingDir, '.opencode', 'agents', '8pm-studio', 'workspace', 'hash', 'performer-1--build.md')
        const oldCodexRelativePath = '.codex/agents/8pm_studio_old_name.toml'
        const oldCodexPath = path.join(workingDir, oldCodexRelativePath)
        const codexRelativePath = '.codex/agents/8pm_studio_performer.toml'
        const codexPath = path.join(workingDir, codexRelativePath)
        ...
        await fs.writeFile(
            path.join(workingDir, '.opencode', '8pm-studio.manifest.json'),
            JSON.stringify({
                version: 1,
                owner: '8pm-studio',
                workspaceHash: 'hash',
        ...
        expect(projectedSkill.codexFilePath).toContain(`${path.sep}.agents${path.sep}skills${path.sep}8pm-studio-performer-1-draft-dance-`)
        expect(projectedSkill.codexFilePath).toMatch(/SKILL\.md$/)
        expect(projectedSkill.codexLinkRelativePath).toMatch(/^\.agents\/skills\/8pm-studio-performer-1-draft-dance-/)
    ...
    it('removes stale Codex project agents immediately when the performer model is unsupported', async () => {
        const oldCodexRelativePath = '.codex/agents/8pm_studio_old_name.toml'
        const oldCodexPath = path.join(workingDir, oldCodexRelativePath)
        const oldSkillLinkRelativePath = '.agents/skills/8pm-studio-old-skill'
    ...
    it('prunes stale performer agent files from the manifest', async () => {
        ...
        const activeBuild = path.join(workingDir, '.opencode', 'agents', '8pm-studio', 'workspace', workspaceHash, 'performer-1--build.md')
        const activePlan = path.join(workingDir, '.opencode', 'agents', '8pm-studio', 'workspace', workspaceHash, 'performer-1--plan.md')
        const staleBuild = path.join(workingDir, '.opencode', 'agents', '8pm-studio', 'workspace', workspaceHash, 'performer-2--build.md')
        const stalePlan = path.join(workingDir, '.opencode', 'agents', '8pm-studio', 'workspace', workspaceHash, 'performer-2--plan.md')
        ...
        await fs.writeFile(
            ...
            JSON.stringify({
                ...
                groups: {
                    'performer:performer-1': [
                        '.opencode/agents/8pm-studio/workspace/hash/performer-1--build.md',
                        '.opencode/agents/8pm-studio/workspace/hash/performer-1--plan.md',
                    ],
                    'performer:performer-2': [
                        '.opencode/agents/8pm-studio/workspace/hash/performer-2--build.md',
                        '.opencode/agents/8pm-studio/workspace/hash/performer-2--plan.md',
                    ],
                },
            }, null, 2),
            'utf-8',
