...
describe('ensurePerformerProjection source boundaries', () => {
    ...
    beforeEach(async () => {
        ...
        compilePerformerMock.mockReset().mockResolvedValue({
            ...
            agentNames: { build: 'apm-studio/workspace/hash/performer-1--build' },
            agentPaths: {
                build: path.join(workingDir, '.opencode', 'agents', 'apm-studio', 'workspace', 'hash', 'performer-1--build.md'),
            },
            agentContents: {
            ...
            skills: [],
            projectionHash: 'hash',
            allFiles: ['.opencode/agents/apm-studio/workspace/hash/performer-1--build.md'],
        })
    ...
    it('prunes stale performer agent files from the manifest', async () => {
        ...
        const activeBuild = path.join(workingDir, '.opencode', 'agents', 'apm-studio', 'workspace', workspaceHash, 'performer-1--build.md')
        const activePlan = path.join(workingDir, '.opencode', 'agents', 'apm-studio', 'workspace', workspaceHash, 'performer-1--plan.md')
        const staleBuild = path.join(workingDir, '.opencode', 'agents', 'apm-studio', 'workspace', workspaceHash, 'performer-2--build.md')
        const stalePlan = path.join(workingDir, '.opencode', 'agents', 'apm-studio', 'workspace', workspaceHash, 'performer-2--plan.md')
            ...
            JSON.stringify({
                ...
                groups: {
                    'performer:performer-1': [
                        '.opencode/agents/apm-studio/workspace/hash/performer-1--build.md',
                        '.opencode/agents/apm-studio/workspace/hash/performer-1--plan.md',
                    ],
                    'performer:performer-2': [
                        '.opencode/agents/apm-studio/workspace/hash/performer-2--build.md',
                        '.opencode/agents/apm-studio/workspace/hash/performer-2--plan.md',
                    ],
                },
            }, null, 2),
            'utf-8',
