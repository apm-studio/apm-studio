...
function eventStream(events: ReadonlyArray<unknown>): AsyncIterable<unknown> {
    return {
        [Symbol.asyncIterator]() {
            let index = 0
            return {
...
describe('buildStudioChatEventStream', () => {
    ...
    it('subscribes to OpenCode events with an isolated abort signal', async () => {
        ...
        const { buildStudioChatEventStream } = await import('./event-stream-service.js')
    ...
    it('auto-accepts Team permissions through the current OpenCode permission reply API', async () => {
