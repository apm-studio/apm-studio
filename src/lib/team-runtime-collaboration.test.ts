...
import { parseTeamSessionOwnerId } from '../../server/lib/session-execution.js'
import { buildTeamContext } from '../../server/services/team-runtime/team-context-builder.js'
import { getStaticTeamTools } from '../../server/services/team-runtime/team-tools.js'
import { Mailbox } from '../../server/services/team-runtime/mailbox.js'
import type { WakeUpTarget } from '../../server/services/team-runtime/event-router.js'
import { buildWakePrompt } from '../../server/services/team-runtime/wake-prompt-builder.js'

const teamDefinition: TeamDefinition = {
    id: 'team-review',
    name: 'Review Team',
    description: 'Ship the final review with clear handoffs.',
    teamRules: ['Keep updates concise.', 'Escalate blockers quickly.'],
    participants: {
        'participant-lead': {
        ...
        'participant-researcher': {
            agentRef: { kind: 'draft', draftId: 'researcher-agent' },
        ...
        'participant-observer': {
            agentRef: { kind: 'draft', draftId: 'observer-agent' },
...
describe('team collaboration rewrite', () => {
    it('builds stable collaboration context without model-facing IDs', () => {
        const context = buildTeamContext(teamDefinition, 'participant-lead')

        expect(context).toContain('# Team Runtime Context')
        ...
        expect(context).not.toContain('Team ID')
    ...
    it('builds wake prompts from transient updates only', () => {
        ...
        expect(prompt).not.toContain('# Team Runtime Context')
    ...
    it('generates session-bound collaboration tools', () => {
        const tools = getStaticTeamTools('/tmp/workspace')
        ...
        expect(messageTool?.content).toContain('/api/team/session/${encodeURIComponent(sessionID)}/message-teammate')
        expect(messageTool?.content).toContain('Pass the teammate display name as recipient')
        expect(boardTool?.content).toContain('/api/team/session/${encodeURIComponent(sessionID)}/update-shared-board')
        ...
        expect(listTool?.content).toContain('/api/team/session/${encodeURIComponent(sessionID)}/list-shared-board')
        expect(listTool?.content).toContain('summary", "full')
        expect(getTool?.content).toContain('/api/team/session/${encodeURIComponent(sessionID)}/get-shared-board-entry')
        expect(waitTool?.content).toContain('/api/team/session/${encodeURIComponent(sessionID)}/wait-until')
        ...
        expect(messageTool?.content).not.toContain('const teamId =')
        expect(messageTool?.content).not.toContain('const threadId =')
    })

    it('parses team participant owner IDs from session context', () => {
        expect(parseTeamSessionOwnerId('team:team-review:thread:thread-1:participant:participant-lead')).toEqual({
            teamId: 'team-review',
            threadId: 'thread-1',
            participantKey: 'participant-lead',
        })
        expect(parseTeamSessionOwnerId('agent:solo')).toBeNull()
    })
})
