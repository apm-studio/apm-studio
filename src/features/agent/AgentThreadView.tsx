...
import { normalizeSessionDiffEntries } from '../chat/session-review-diffs'
...
type Props = {
    ...
    onOpenRevert: (agentId: string, messageId: string, content: string) => void
...
export default function AgentThreadView({
    ...
    const chatSession = useChatSession(agentId)
    ...
    useEffect(() => {
        ...
        let active = true
        void getDiff(agentId).then((entries) => {
            if (active) {
                setSessionDiffState({
                    sessionId,
                    entries: entries || [],
    ...
    const hasDiffs = sessionDiffs.length > 0
    ...
    const handleRestoreRevertedMessage = useCallback((messageId: string) => {
        void restoreRevertedMessage(agentId, messageId)
    }, [agentId, restoreRevertedMessage])
    ...
    }, [hasActiveSession, isLoading, isMutating, onOpenRevert, agentId, prefixCount, visibleMessages])
