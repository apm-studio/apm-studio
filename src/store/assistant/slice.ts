 ...
 * All chat logic (session, sending, streaming) is delegated to the chat slice.
 *
...
import type { AssistantAvailableModel, AssistantModelSelection, AssistantSlice } from './types'
...
export const ASSISTANT_AGENT_ID = ASSISTANT_CHAT_OWNER_ID
export { buildAssistantChatKey, isAssistantChatKey }

function sameAssistantModel(
    left: AssistantModelSelection | null,
    right: AssistantModelSelection | null,
) {
...
function sameAvailableModels(
    left: AssistantAvailableModel[],
    right: AssistantAvailableModel[],
) {
    if (left === right) return true
