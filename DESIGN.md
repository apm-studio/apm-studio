...
Export should use two clear pairings from the shared left sidebar. Turning on `APM User` opens `User <> Workspace` copy mode; turning it off, or choosing a workspace, opens `Workspace <> Targets` export mode. Workspace packages can be staged for target export by dragging or using `Add`; User packages should first be staged as a copy into Workspace. Dropping, adding, or copying only stages changes; the header `Save` button writes the active pairing's package copies or target files, and `Revert` discards the active pairing's staged changes back to the current state. Source cards should make target state visible with `Synced`, `Unsynced`, `Staged`, or `Blocked` status in target export mode so users can see what still needs explicit save before they drag anything. Targets should read the selected environment's definition files and show staged Save, Skip, Keep, Current, Managed, and target-only items next to the Studio source so users choose the export action from the comparison itself instead of parsing repeated summary text. Target-only rows may expose `Import` as a secondary row action that creates a Workspace APM package from the unmanaged target file; it must not claim target ownership until a later explicit Export save. Export writes are always by APM primitive unit: Agents, Instructions, Skills, Prompts, Commands, Hooks, or MCP. Do not introduce a separate Studio Agent export unit.
